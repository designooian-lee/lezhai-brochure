...
async function main() {
  ...
  const videoAttachment=new FormData();videoAttachment.append('_csrf',csrf);videoAttachment.append('media_type','video');videoAttachment.append('source_type','upload');videoAttachment.append('media_title','14MB MP4 测试文件');videoAttachment.append('media_sort_order','2');videoAttachment.append('media',new Blob([Buffer.from([0,0,0,24]),Buffer.from('ftypisom'),Buffer.alloc(14*1024*1024)],{type:'application/octet-stream'}),'large.mp4');
  const videoUpload=await fetch(`${adminBase}/tutorials/${tutorialId}/media`,{method:'POST',headers:{cookie:adminJar.join('; ')},body:videoAttachment});const videoResult=await videoUpload.json();assert(videoUpload.status===200&&videoResult.media.file_path.endsWith('.mp4'),'大于 8MB 且 MIME 被识别为二进制的 MP4 可按文件头安全上传');
  ...
  const deleteVideo=await fetch(`${siteRoot}${videoResult.delete_url}`,{method:'POST',headers:{cookie:adminJar.join('; '),'content-type':'application/x-www-form-urlencoded'},body:new URLSearchParams({_csrf:csrf})});assert(deleteVideo.status===200&&((await deleteVideo.json()).ok),'大文件视频附件可删除');
