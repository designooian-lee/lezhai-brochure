...
final class WebsiteApp
{
    ...
    public function renderArticle(array $article,bool $preview=false): void
    {
        ...
        $cover=$article['cover_path']?'<img class="website-article-cover" src="'.e($article['cover_path']).'" alt="'.e($article['title']).'">':'';
        ...
        $hotHtml=$hotLinks===''?'<p class="article-hot-empty">暂无其他文章</p>':'<div class="article-hot-viewport"><div class="article-hot-track">'.$hotLinks.'<div aria-hidden="true" class="article-hot-copy">'.$hotLinks.'</div></div></div>';
        $content='<section class="content-hero"><div class="container content-hero-box"><p class="eyebrow">LEZHAI JOURNAL</p><h1 class="page-title">'.e($article['title']).'</h1><p class="lead">'.e($article['excerpt']).'</p><p class="article-date">'.e($date).'</p></div></section><section class="section"><div class="container content-grid"><article class="content-main prose website-richtext">'.$cover.$article['body_html'].$neighborHtml.'</article><aside class="content-side info-panel"><img src="/brand/logo-signature.png" alt="乐宅.Life，把喜欢的门，装进生活"><dl><dt>文章状态</dt><dd>'.($preview?'管理员预览':'已发布').'</dd><dt>更新时间</dt><dd>'.e(date('Y-m-d',strtotime((string)$article['updated_at']))).'</dd></dl><section class="article-hot" aria-labelledby="article-hot-title"><h2 id="article-hot-title">热门文章</h2>'.$hotHtml.'</section></aside></div></section>';
        ...
        $this->dynamicShell($title,$description,$content,'/articles/'.$article['slug'],$article['cover_path']?:'/images/og-lezhai.jpg',$schema,$preview);
    }
    ...
    private function dynamicShell(string $title,string $description,string $content,string $path,string $image='/images/og-lezhai.jpg',?array $schema=null,bool $preview=false): void
    {
