...
final class App
{
    ...
    public function run(string $path): void
    {
        ...
        try {
            if ($path === '/health') {
            ...
            } elseif (preg_match('~^/source-pages/(\d+)/(\d+)$~', $path, $m) && $method === 'GET') {
                $this->sourcePage((int) $m[1], (int) $m[2]);
            ...
            } elseif (preg_match('~^/admin/tutorials/(\d+)/media$~', $path, $m) && $method==='POST') {
                Auth::requireLogin(); Auth::verifyCsrf(); $this->tutorialMediaAdd((int)$m[1]);
            } elseif (preg_match('~^/admin/tutorials/(\d+)/media/order$~', $path, $m) && $method==='POST') {
                Auth::requireLogin(); Auth::verifyCsrf(); $this->tutorialMediaOrder((int)$m[1]);
            } elseif (preg_match('~^/admin/tutorial-media/(\d+)/delete$~', $path, $m) && $method==='POST') {
                Auth::requireLogin(); Auth::verifyCsrf(); $this->tutorials->deleteMedia((int)$m[1]); $this->json(['ok'=>true]);
            ...
            } elseif ($path === '/admin/catalog-parse-jobs' && $method==='POST') {
                Auth::requireLogin(); Auth::verifyCsrf(); $this->catalogParseEnqueue();
            } elseif (preg_match('~^/admin/catalog-parse-jobs/(\d+)$~',$path,$m)&&$method==='GET') {
                Auth::requireLogin(); $this->catalogParseStatus((int)$m[1]);
                ...
                Auth::requireLogin(); Auth::verifyCsrf(); $this->queueCatalogJob((int) $m[1]);
            } elseif (preg_match('~^/admin/catalogs/(\d+)/local-pages$~', $path, $m) && $method === 'POST') {
                Auth::requireLogin(); Auth::verifyCsrf(); $this->queueCatalogJob((int) $m[1]);
            } elseif (preg_match('~^/admin/catalogs/(\d+)/job$~', $path, $m) && $method === 'GET') {
                Auth::requireLogin(); $this->catalogJobStatus((int)$m[1]);
            } elseif (preg_match('~^/admin/catalogs/(\d+)/download-ready$~', $path, $m) && $method === 'GET') {
                Auth::requireLogin(); $this->download((int)$m[1]);
            } elseif (preg_match('~^/admin/catalogs/(\d+)/local-pages/delete$~', $path, $m) && $method === 'POST') {
                Auth::requireLogin(); Auth::verifyCsrf(); $this->catalogs->deleteLocalPages((int)$m[1]); $this->flash('本地高清图片和下载包已删除。'); header('Location: '.base_path('admin/catalogs/'.(int)$m[1].'/edit')); exit;
            } else {
    ...
    {
        $neighbors=$this->articles->neighbors($article);ob_start();?><header class="site-header"><a class="brand-placeholder" href="<?=e(base_path())?>"><span class="brand-cn">乐宅.Life</span></a></header><?=$this->publicNav('articles')?><main class="tutorial-detail"><a class="text-link" href="<?=e(base_path('articles'))?>">← 返回文章列表</a><article><div class="tutorial-title"><span class="eyebrow">JOURNAL</span><h1><?=e($article['title'])?></h1><p><?=e($article['excerpt'])?></p></div><div class="article-body rich-html"><?=$article['body_html']?></div><?=$this->articleNeighbors($neighbors,true)?></article></main><?php $head='<link rel="canonical" href="https://lezhai.life/articles/'.e($article['slug']).'">'.(!empty($article['seo_keywords'])?'<meta name="keywords" content="'.e($article['seo_keywords']).'">':'');$this->layout($article['seo_title']?:$article['title'],(string)ob_get_clean(),false,$head);
    }
    ...
    private function reader(int $id): void
    {
        ...
        if ($catalog['reader_mode'] === 'local') {
            ...
            $sourcePages = json_decode($catalog['page_manifest'], true) ?: [];
            foreach (array_keys($sourcePages) as $index) $pages[] = base_path('source-pages/' . $id . '/' . $index);
        }
        if ($pages !== []) {
    ...
    private function sourcePage(int $id, int $index): never
    {
        $file = $this->catalogs->sourcePageFile($id, $index);
        if (!$file) { http_response_code(404); exit('页面不存在。'); }
        $type = (new \finfo(FILEINFO_MIME_TYPE))->file($file) ?: 'application/octet-stream';
        header('Content-Type: ' . $type);
        header('Content-Length: ' . filesize($file));
    ...
    private function articleForm(string $method, ?int $id=null): void
    {
        ...
        if($method==='POST'){try{Auth::verifyCsrf();$saved=$this->articles->save($_POST,$_FILES['cover']??null,$id);$this->flash($id?'文章已更新。':'文章已创建。');$location=($_POST['status']??'draft')==='published'?base_path('admin'):base_path('admin/articles/'.$saved.'/edit');header('Location: '.$location);exit;}catch(\Throwable $exception){$error=$exception->getMessage();$article=array_merge($article??[],$_POST);}}
        ob_start();?><main class="admin-shell"><?=$this->adminHeader($id?'编辑文章':'写文章')?><?php if($error):?><div class="notice error"><?=e($error)?></div><?php endif?><section class="form-card"><form method="post" enctype="multipart/form-data" data-article-form><input type="hidden" name="_csrf" value="<?=e(Auth::csrf())?>">
        <label>文章标题<input name="title" maxlength="180" required value="<?=e($article['title']??'')?>"></label><label>英文网址标识（可选）<input name="slug" maxlength="180" pattern="[a-z0-9-]*" placeholder="留空自动生成 article-编号" value="<?=e($article['slug']??'')?>"><small>留空将自动生成；发布后请勿修改，以免已有链接失效。</small></label><label>文章摘要<textarea name="excerpt" maxlength="500" rows="3"><?=e($article['excerpt']??'')?></textarea></label>
        <label>封面图片<input type="file" name="cover" accept="image/jpeg,image/png,image/webp,image/gif"><small>建议尺寸：500 × 670 PX，JPG/PNG/WebP，最大 8MB；未上传时自动使用正文第一张图片。</small></label><?php if(!empty($article['cover_path'])):?><label class="check"><input type="checkbox" name="remove_cover" value="1"> 删除现有封面</label><?php endif?>
        <div class="rich-editor-field"><span>文章正文</span><div class="editor-toolbar" role="toolbar"><button type="button" data-command="bold">加粗</button><button type="button" data-block="h2">标题</button><button type="button" data-command="insertUnorderedList">列表</button><button type="button" data-link>链接</button><button type="button" data-image>插入图片</button><input type="file" accept="image/jpeg,image/png,image/webp,image/gif" data-image-input hidden></div><div class="rich-editor" contenteditable="true" data-editor><?=($article['body_html']??'')?></div><textarea name="body_html" data-editor-source hidden><?=e($article['body_html']??'')?></textarea><small>支持选择、粘贴或拖拽图片；图片会自动压缩并保存到本站。</small></div>
        <label>SEO 关键字<input name="seo_keywords" maxlength="300" value="<?=e($article['seo_keywords']??'')?>"><small>多个关键字用英文逗号分隔。</small></label><label>SEO 标题<input name="seo_title" maxlength="180" value="<?=e($article['seo_title']??'')?>"><small>留空时使用文章标题。</small></label><label>Meta Description<textarea name="meta_description" maxlength="300" rows="3"><?=e($article['meta_description']??'')?></textarea><small>留空时使用文章摘要。</small></label><label>发布状态<select name="status"><option value="draft" <?=($article['status']??'draft')==='draft'?'selected':''?>>草稿</option><option value="published" <?=($article['status']??'')==='published'?'selected':''?>>发布</option></select></label><div class="form-actions"><button class="button">保存文章</button><?php if($article):?><a class="button secondary" target="_blank" href="<?=e(base_path('admin/articles/'.$article['id'].'/preview?surface=website'))?>">预览官网版本</a><a class="button secondary" target="_blank" href="<?=e(base_path('admin/articles/'.$article['id'].'/preview?surface=brochure'))?>">预览图册版本</a><?php endif?><a class="button secondary" href="<?=e(base_path('admin'))?>">返回后台</a></div></form></section></main><?php $this->layout($id?'编辑文章':'写文章',(string)ob_get_clean(),true);
    }
    ...
    private function tutorialForm(string $method, ?int $id=null): void
    {
        ...
        ob_start();?><main class="admin-shell"><?=$this->adminHeader($id?'编辑教程':'添加教程')?><section class="form-card"><form method="post" enctype="multipart/form-data"><input type="hidden" name="_csrf" value="<?=e(Auth::csrf())?>"><label>教程标题<input name="title" maxlength="180" required value="<?=e($t['title']??'')?>"></label><label>一句话介绍<textarea name="description" maxlength="300" rows="3"><?=e($t['description']??'')?></textarea></label><label>文章正文<textarea name="body" rows="12"><?=e($t['body']??'')?></textarea><small>使用空行分段，前台会保留换行。</small></label><label>封面图片<input type="file" name="cover" accept="image/jpeg,image/png,image/webp"></label><label>人工排序值<input type="number" name="manual_priority" value="<?=(int)($t['manual_priority']??0)?>"></label><label class="check"><input type="checkbox" name="is_active" <?=!isset($t)||$t['is_active']?'checked':''?>> 在客户页面显示</label><div class="form-actions"><button class="button">保存教程</button><a class="button secondary" href="<?=e(base_path('admin'))?>">返回后台</a></div></form></section><?php if($t):?><section class="admin-section" data-tutorial-media><h2>附件</h2><form class="media-fields" data-media-upload action="<?=e(base_path('admin/tutorials/'.$id.'/media'))?>" method="post" enctype="multipart/form-data"><input type="hidden" name="_csrf" value="<?=e(Auth::csrf())?>"><label>类型<select name="media_type"><option value="video">视频</option><option value="document">文档</option></select></label><label>来源<select name="source_type"><option value="external">外部链接</option><option value="upload">自己上传</option></select></label><label>附件标题<input name="media_title" maxlength="180"></label><label data-media-url>外部链接<input type="url" name="media_url" placeholder="https://"></label><label data-media-file hidden>上传文件<input type="file" name="media" accept="video/mp4,video/webm,application/pdf,.doc,.docx"><small>视频支持 MP4/WebM，文档支持 PDF/Word，单文件最大 500MB。</small></label><label>附件排序<input type="number" name="media_sort_order" value="0"></label><div class="upload-progress" data-upload-progress hidden><progress value="0" max="100"></progress><span>0%</span></div><p class="error-text" data-media-error hidden></p><button class="button" type="submit" data-media-submit>添加外链附件</button></form><form data-media-order action="<?=e(base_path('admin/tutorials/'.$id.'/media/order'))?>" method="post"><input type="hidden" name="_csrf" value="<?=e(Auth::csrf())?>"><div class="table-wrap"><table><tbody data-media-list><?php foreach($t['media'] as $m):?><tr data-media-id="<?=(int)$m['id']?>"><td data-media-title><?=e($m['title']?:'未命名附件')?></td><td><?=e($m['media_type']==='video'?'视频':'文档')?> · <?=e($m['source_type']==='upload'?'上传':'外链')?></td><td><input class="media-order-input" type="number" name="orders[<?=(int)$m['id']?>]" value="<?=(int)$m['sort_order']?>" aria-label="附件排序"></td><td class="actions"><button type="button" data-media-delete-url="<?=e(base_path('admin/tutorial-media/'.$m['id'].'/delete'))?>">删除</button></td></tr><?php endforeach?></tbody></table></div><div class="form-actions"><button class="button secondary" type="submit">保存附件排序</button><span data-order-message></span></div></form></section><?php endif?></main><?php $this->layout($id?'编辑教程':'添加教程',(string)ob_get_clean(),true);
    }

    private function tutorialMediaAdd(int $id): never
    {
        try{$media=$this->tutorials->addMedia($id,$_POST,$_FILES['media']??[]);$this->json(['media'=>$media,'delete_url'=>base_path('admin/tutorial-media/'.$media['id'].'/delete')]);}catch(\Throwable $exception){http_response_code(str_contains($exception->getMessage(),'500MB')?413:422);$this->json(['error'=>$exception->getMessage()]);}
    }

    private function tutorialMediaOrder(int $id): never
    {
        try{$this->tutorials->reorderMedia($id,is_array($_POST['orders']??null)?$_POST['orders']:[]);$this->json(['ok'=>true]);}catch(\Throwable $exception){http_response_code(422);$this->json(['error'=>$exception->getMessage()]);}
    }
    ...
    private function catalogForm(?array $catalog = null, ?array $preview = null, string $error = ''): void
    {
        ...
        <?php if ($editing): ?><section class="local-pages-panel" data-catalog-job data-status-url="<?=e(base_path('admin/catalogs/'.$catalog['id'].'/job'))?>"><div><strong>本地高清页面</strong><p data-job-message><?= (int)$catalog['local_page_count']>0?'已生成 '.(int)$catalog['local_page_count'].' 张，可选择本地图片阅读。':'尚未生成；当前只能使用原网页阅读。' ?></p><progress data-job-progress value="0" max="1" hidden></progress><p class="error-text" data-job-error hidden></p></div><div class="local-pages-actions"><form method="post" action="<?=e(base_path('admin/catalogs/'.$catalog['id'].'/local-pages'))?>"><input type="hidden" name="_csrf" value="<?=e(Auth::csrf())?>"><button class="button secondary" type="submit"><?= (int)$catalog['local_page_count']>0?'重新生成本地图片':'生成本地图片' ?></button></form><a class="button secondary" data-job-download href="<?=e(base_path('admin/catalogs/'.$catalog['id'].'/download-ready'))?>" <?= $this->catalogs->readyDownload((int)$catalog['id'])?'':'hidden' ?>>下载 ZIP</a><?php if((int)$catalog['local_page_count']>0):?><form method="post" action="<?=e(base_path('admin/catalogs/'.$catalog['id'].'/local-pages/delete'))?>" onsubmit="return confirm('确定删除全部本地高清图片和下载包吗？')"><input type="hidden" name="_csrf" value="<?=e(Auth::csrf())?>"><button class="button secondary danger" type="submit">删除图片</button></form><?php endif?></div></section><?php endif; ?>
        <section class="form-card"><form method="post" enctype="multipart/form-data" <?=!$editing?'data-catalog-parse-form data-parse-url="'.e(base_path('admin/catalog-parse-jobs')).'" data-status-base="'.e(base_path('admin/catalog-parse-jobs/')).'"':''?>><input type="hidden" name="_csrf" value="<?= e(Auth::csrf()) ?>"><?php if (!$editing): ?><input type="hidden" name="parse_job_id" value=""><label>电子图册链接<input type="url" name="source_url" required value="<?= e($_POST['source_url'] ?? '') ?>" placeholder="粘贴云展网、goootu 或 FLBOOK 链接"></label><div class="parse-job-panel" data-parse-panel hidden><p data-parse-message></p><progress data-parse-progress></progress><p class="error-text" data-parse-error hidden></p></div><div class="parse-preview" data-parse-preview hidden><img src="" alt="解析到的封面"><div><span class="status ok" data-parse-pages></span><h2 data-parse-title></h2><p data-parse-description></p><small data-parse-source></small></div></div><?php else: ?><div class="readonly-field"><span>来源链接</span><code><?= e($catalog['source_url']) ?></code></div><label>前端阅读方式<select name="reader_mode"><option value="source" <?= $catalog['reader_mode'] === 'source' ? 'selected' : '' ?>>原网页直接显示</option><option value="local" <?= $catalog['reader_mode'] === 'local' ? 'selected' : '' ?> <?= (int) $catalog['local_page_count'] < 1 ? 'disabled' : '' ?>>本地高清图片<?= (int) $catalog['local_page_count'] < 1 ? '（请先生成）' : '' ?></option></select><small>本地图片阅读加载更稳定；重新解析图册后需重新生成。</small></label><?php endif; ?><label>分类<select name="category_id" required><option value="">请选择</option><?php foreach ($categories as $c): $selected=(int)($catalog['category_id'] ?? $_POST['category_id'] ?? 0)===(int)$c['id']; ?><option value="<?= $c['id'] ?>" <?= $selected?'selected':'' ?>><?= e($c['name']) ?></option><?php endforeach; ?></select></label><label>图册名称<input name="name" maxlength="180" <?= $editing ? 'required' : '' ?> value="<?= e($catalog['name'] ?? $_POST['name'] ?? '') ?>"></label><label>一句话介绍<textarea name="description" maxlength="300" rows="3"><?= e($catalog['description'] ?? $_POST['description'] ?? '') ?></textarea></label><label>人工排序值<input type="number" name="manual_priority" value="<?= (int)($catalog['manual_priority'] ?? $_POST['manual_priority'] ?? 0) ?>"><small>数值越大越靠前；相同数值再按热度。</small></label><?php if ($editing): ?><label>手工替换封面<input type="file" name="cover" accept="image/jpeg,image/png,image/webp"></label><?php endif; ?><label class="check"><input type="checkbox" name="is_active" <?= !isset($catalog) || $catalog['is_active'] ? 'checked' : '' ?>> 在客户页面显示</label><div class="form-actions"><?php if ($editing): ?><button class="button" type="submit">保存修改</button><?php else: ?><button class="button" type="button" data-parse-submit>解析链接</button><button class="button" type="submit" name="action" value="save" data-catalog-save hidden>确认添加</button><?php endif; ?><a class="button secondary" href="<?= e(base_path('admin')) ?>">取消</a></div></form></section></main>
        <?php $this->layout($editing ? '编辑图册' : '添加图册', (string) ob_get_clean(), true);
    }

    private function download(int $id): void
    {
        $file = $this->catalogs->readyDownload($id);
        if (!$file) throw new RuntimeException('高清下载包尚未生成完成。');
    ...
    private function queueCatalogJob(int $id): never
    {
        if (!$this->catalogs->find($id,true)) throw new RuntimeException('图册不存在。');
        $this->jobs->enqueue($id);$this->flash('高清页面已加入后台生成队列。');
        header('Location: '.base_path('admin/catalogs/'.$id.'/edit'));exit;
    }

    private function catalogJobStatus(int $id): never
    {
        $job=$this->jobs->latest($id);$catalog=$this->catalogs->find($id,true);
        $this->json(['job'=>$job,'local_page_count'=>(int)($catalog['local_page_count']??0),'download_url'=>$this->catalogs->readyDownload($id)?base_path('admin/catalogs/'.$id.'/download-ready'):null]);
    }

    private function catalogParseEnqueue(): never
    {
        try{$job=$this->jobs->enqueueParse((string)($_POST['source_url']??''));http_response_code(202);$this->json(['job_id'=>(int)$job['id']]);}catch(\Throwable $exception){http_response_code(422);$this->json(['error'=>$exception->getMessage()]);}
    }

    private function catalogParseStatus(int $id): never
    {
        $job=$this->jobs->find($id);if(!$job||$job['job_type']!=='parse'){http_response_code(404);$this->json(['error'=>'解析任务不存在。']);}
        $result=$job['status']==='completed'?json_decode((string)$job['result_payload'],true):null;
        $this->json(['job'=>['id'=>(int)$job['id'],'status'=>$job['status'],'phase'=>$job['phase'],'error'=>$job['error']],'result'=>$result]);
    }
